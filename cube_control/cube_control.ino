...
// Calculating jerk for cube move events
const int JERK_WINDOW = 200; // time in milliseconds for averaging jerk
const int TIMESTEP = 50; // time in milleseconds between sensor reads
double jerk_x = 0.0;
double jerk_y = 0.0;
double jerk_z = 0.0;
...
//  Serial.print("  \tZ:  "); Serial.print(lis.z);
  ...
  /* Calculate jerk */
  jerk_y = curr_y - prev_y;
  jerk_x = curr_x - prev_x;
  jerk_z = curr_z - prev_z;
  ...
  Serial.print(" \tjerk x: "); Serial.println(jerk_x);
  Serial.print(" \tjerk y: "); Serial.println(jerk_x);
  Serial.print(" \tjerk z: "); Serial.println(jerk_x);

  if(jerk_y > 5){                      //jerk in accel y > 10
    Serial.println("jolt y up");
  }
  else if (jerk_y < -5){             //jerk in accel y < -10
       Serial.println("jolt y down");
       // Serial1.println("jolt y down");
        Serial1.println("DOWN");
  } else if (jerk_x < -5){             //jerk in accel x < -10
